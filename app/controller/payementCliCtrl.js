...
const datediff = async(req, res)=>{
    ...
    if(deb==undefined && fi!==undefined){
        ...
        sql="SELECT payementclis.id, payee,datepayement,date_echeance,net,idcommande as numfact,net,clients.name as clien from commandeclis, clients, payementclis,listecomms where commandeclis.idcli= clients.id AND commandeclis.id=listecomms.idcomm AND listecomms.idcomm=payementclis.idcommande AND payementclis.datepayement BETWEEN '"+deb+"' and '"+fi+"' GROUP BY numfact ORDER BY id DESC  "
